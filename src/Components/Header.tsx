...
const Header = ( )  =>{
    ...
    return (
        <header className="w-full px-4 flex justify-between items-center bg-[var(--card)] text-[var(--text)] border-b border-[var(--border)] transition-colors duration-300" >
            ...
            <IoLanguageSharp aria-label="change-language" className="text-lg cursor-pointer text-[var(--muted)] hover:text-[var(--accent)] transition-colors"  onClick={ handleLanguage }/>
        </header>
    )
}
